...
notes-md-skill — standalone Claude Code skill plugin providing the `/notes` command for living development notes as institutional memory.

## Key Files

- `NOTES.md` — institutional memory (decisions, rationale, conventions)
- `skills/notes/SKILL.md` — the `/notes` skill definition
...
When the `/notes` skill behavior changes, bump the **plugin** version in both `.claude-plugin/plugin.json` (`version` field) and `.claude-plugin/marketplace.json` (the corresponding entry's `version` in the `plugins` array). These two must stay in sync. Do **not** bump the marketplace-level `metadata.version` — that tracks the marketplace listing itself, not the plugin.

- **PATCH** (0.1.x): bug fixes, wording tweaks, formatting changes to NOTES.md handling, internal refactors
- **MINOR** (0.x.0): new subcommands, new NOTES.md sections, meaningful feature additions
- **MAJOR** (x.0.0): breaking changes to NOTES.md format, removed subcommands, anything that would surprise an existing user
...
- **MINOR**: adding a new companion plugin to the `plugins` array
- **MAJOR**: removing a plugin, restructuring the listing in a breaking way
